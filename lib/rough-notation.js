...
function markAnnotationDirty(annotation) {
    dirtyAnnotations.add(annotation);
    scheduleUpdate();
}
class RoughAnnotationImpl {
    constructor(e, config) {
        this._state = "unattached"; // Made public for batching system
    ...
    hasOnlyPositionChanged(newRects) {
    ...
    translateSVGContent(newRects) {
        if (!this._svg || this._lastSizes.length === 0)
            return;
        if (newRects.length === 1 && this._lastSizes.length === 1) {
            ...
            const isConsistent = newRects.every((rect, i) => {
                ...
                return (Math.abs(deltaX - avgDeltaX) < 2 && Math.abs(deltaY - avgDeltaY) < 2);
            });
            if (isConsistent) {
    ...
    applyTranslationToAllPaths(deltaX, deltaY) {
        if (!this._svg)
            return;
        ...
        paths.forEach((path) => {
            ...
            if (currentTransform.includes("translate")) {
                ...
                if (translateMatch) {
                    ...
                    newTransform = currentTransform.replace(/translate\([^)]+\)/, `translate(${currentX + deltaX}, ${currentY + deltaY})`);
                }
            }
            else {
                // Add new translate
    ...
    hide(force) {
        ...
        const animate = (_a = this.animateOnHide) !== null && _a !== void 0 ? _a : false;
        if (this._svg && !force && animate) {
            const paths = Array.from(this._svg.querySelectorAll("path"));
            if (paths.length > 0) {
                const animationDuration = this._config.animationDuration || DEFAULT_ANIMATION_DURATION;
    ...
    renderWithRects(svg, rects, ensureNoAnimation) {
...
export function annotationGroup(annotations) {
    ...
    for (const a of annotations) {
        ...
        const duration = ai.animationDuration === 0
            ? 0
